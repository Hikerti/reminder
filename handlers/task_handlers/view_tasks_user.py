...
@router.message(Command(commands='viewtasks'))
async def process_view_tasks_list(message: Message):
    ...
    if not tasks: 
        await message.answer('У вас пока нет задач\n\n'
                             'Создайте заметку при помощи комманды /createtask')
        return
    await message.answer('Ваши задачи:')

    for task in tasks:
        text = f"📌 <b>{task.title}</b>\n\n{task.description}"
        await message.answer(text, parse_mode="HTML", reply_markup=keyboard_function_task)